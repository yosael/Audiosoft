...
public class VentaComboHome extends KubeDAO<VentaProdServ> {
	...
	private Integer idCotizacionParaDuplicar;
	
	private Map<Integer,List<CotCmbsItems>> itemsCotizadosPorCombo = new HashMap<Integer, List<CotCmbsItems>>();
	private Map<Integer,List<CotCmbsItems>> itemsCotizadosPorComboBin = new HashMap<Integer, List<CotCmbsItems>>();
			...
			if (getCotizId() != null && getCotizId() > 0) 
			{
				...
				try {
					...
					for (CotizacionCombos tmpCmb : tmpCtCmbs) 
					{
						...
						//nuevo agregado el 01/08/2017
						itemsCotizadosPorCombo.put(tmpCmb.getCombo().getId(), new ArrayList<CotCmbsItems>());
						for(CotCmbsItems itemsCot:itemsCotizados)
						{
							itemsCotizadosPorCombo.get(tmpCmb.getCombo().getId()).add(itemsCot);
						}
						...
						///////////////////////////////////////	
							...
							{
								/*if(tmpItmsCot.getItem()!=null)// se cambiaria a producto
								{*/
								
								if(tmpItmsCot.getProducto()!=null)// se cambiaria a producto
								{
									...
									//if (tmpItmsCot.getItem().getId().equals(tmpItm.getId())) { comentado el 13/07/2017
										...
										//nuevo agregado el 27/07/2017
										ItemComboApa itemCmbTmp = new ItemComboApa();
										itemCmbTmp.setCategoria(tmpItmsCot.getProducto().getCategoria());
										itemCmbTmp.setProducto(tmpItmsCot.getProducto());
										itemCmbTmp.setCantidad(tmpItmsCot.getCantidad());
										itemCmbTmp.setPrincipal(tmpItmsCot.isPrincipal()!=null?tmpItmsCot.isPrincipal():false);
										itemCmbTmp.setPrecioCotizado(tmpItmsCot.getPrecioCotizado());
										itemCmbTmp.setTipoPrecio(tmpItmsCot.getTipoPrecio());
										itemCmbTmp.setTipPreCotizado(tmpItmsCot.getTipoPrecio());
										itemCmbTmp.setGeneraRequisicion(tmpItmsCot.isGeneraRequisicion()!=null?tmpItmsCot.isGeneraRequisicion():false);
										itemCmbTmp.setNombreAdaptacion(tmpItmsCot.getNombreAdaptacion());
										itemCmbTmp.setSelCategoria(tmpItmsCot.getSelCategoria());
										itemCmbTmp.setIdItemCotizado(tmpItmsCot.getId());
										...
										/*tmpItmsCot.getItem().setPrecioCotizado(tmpItmsCot.getPrecioCotizado());comentado el 27/07/2017
										tmpItmsCot.getItem().setTipPreCotizado(tmpItmsCot.getTipoPrecio());*/
										
										//preciosCotizados.add(tmpItmsCot.getItem()); comentado el 27/07/2017
										
										preciosCotizados.add(itemCmbTmp);
				...
				if (tmpCotiz.getHijoBin() != null && tmpCotiz.getHijoBin().size() > 0) 
				{
					...
					try {
						...
						for (CotizacionCombos tmpCmbBin : tmpCtCmbsBin) //3 combos 3 iteraciones maximo
						{
							...
							//nuevo agregado el 01/08/2017
							itemsCotizadosPorComboBin.put(tmpCmbBin.getCombo().getId(), new ArrayList<CotCmbsItems>());
							for(CotCmbsItems itemsCot:itemsCotizadosBin)
							{
								itemsCotizadosPorComboBin.get(tmpCmbBin.getCombo().getId()).add(itemsCot);
							}
...
*/								
								...
								for (CotCmbsItems tmpItmsCot : itemsCotizadosBin) 
								{
									...
									{*/
									if(tmpItmsCot.getProducto()!=null)
									{
										...
										if (tmpItmsCot.getItem().getId().equals(tmpNItem.getId())) 
										{
											...
											preciosCotizadosBin.add(itemCmbTmp);
										//}
...
*/						
			...
			//nuevo agregado el 06/07/2017
			porcentajeIVA=obtenerIVA();// metodo agregado el 26/07/2017
			...
			//Agregar condicion para cargar nueva
			
			if(idCotizacionParaDuplicar!=null)
			{
				duplicarCotizacion();
			}
	...
	public void duplicarCotizacion()
	{
		...
		CotizacionComboApa tmpCotiz = (CotizacionComboApa) getEntityManager()
				.createQuery(
						"SELECT c FROM CotizacionComboApa c WHERE c.id = :idCotiz AND c.cotizacionComboBin = NULL")
				.setParameter("idCotiz", idCotizacionParaDuplicar).getSingleResult();
		
		System.out.println("Tamanio cotiz a duplicar "+tmpCotiz);
		
		instance.setCliente(tmpCotiz.getCliente());
		
		try {
			
			cargarCombosCotizadosParaDuplicar(idCotizacionParaDuplicar);
			
			if(tmpCotiz.getHijoBin() != null && tmpCotiz.getHijoBin().size() > 0) 
			{
				cargarCombosCotizadosParaDuplicarBin(tmpCotiz);
			}
			...
			FacesMessages.instance().add(Severity.WARN,"No se pudieron cargar los combos de la cotizacion");
		}
			
	}
	
	
	public void ignorarCotizacion(CotizacionComboApa coti)
	{
		
		
		System.out.println("ID cotizacion "+coti.getId());
		
		coti.setIgnorar("si");
		
		getEntityManager().merge(coti);
		...
		{
			getCotiPendByDateAdmin();
		}
		else
		{   
			getCotizacionesPendByDate();
		}
		
		FacesMessages.instance().add(Severity.WARN,"Se actualizo la cotizacion");
	}
	...
	//nuevo el 26/07/2017
	public void cargarCombosCotizadosParaDuplicar(Integer idCotizacionPadre) throws Exception
	{

		System.out.println("Entro a cargar combos para duplicar");
		
		// Cargamos la lista de combos cotizados (CotizacionCombos)
		List<CotizacionCombos> tmpCtCmbs = getEntityManager()
				.createQuery(
						"SELECT cc FROM CotizacionCombos cc WHERE cc.cotizacion.id = :cot")
				.setParameter("cot", idCotizacionPadre).getResultList();
		...
		for (CotizacionCombos tmpCmb : tmpCtCmbs) 
		{
			...
			combo = new ComboAparato();
			combo.setCostosCombo(new ArrayList<CostoServicio>()); 
			List<CostoServicio> costosCmb = new ArrayList<CostoServicio>(); 
			
			if(tmpCmb.getCombo().getCostosCombo().size()>0)
			{
				for(CostoServicio cs:tmpCmb.getCombo().getCostosCombo()) 
				{
					costosCmb.add(cs);
				}
			}
			...
			combo.setCodigo(tmpCmb.getCombo().getCodigo());
			combo.setDescripcion(tmpCmb.getCombo().getDescripcion());
			combo.setPeriodoGarantia(tmpCmb.getCombo().getPeriodoGarantia());
			combo.setNombre(tmpCmb.getCombo().getNombre());
			combo.setEstado(tmpCmb.getCombo().getEstado());
			combo.setEmpresa(tmpCmb.getCombo().getEmpresa());
	...
	public void cargarCombosCotizadosParaDuplicarBin(CotizacionComboApa cotizPadre) throws Exception
	{
		
		CotizacionComboApa tmpBin = cotizPadre.getHijoBin().get(0);
		setBinaural(true);
		aparatoClienteHome.getInstance().setLadoAparatoBin(tmpBin.getLadoAparato());					

		List<CotizacionCombos> tmpCtCmbsBin = getEntityManager().createQuery("SELECT cc FROM CotizacionCombos cc WHERE cc.cotizacion.id = :cotBin")
				.setParameter("cotBin", tmpBin.getId())
				.getResultList();
		
			ComboAparato comboBin;
			List<ItemComboApa> preciosCotizadosBin = new ArrayList<ItemComboApa>();int conteoBin=0;
			for (CotizacionCombos tmpCmbBin : tmpCtCmbsBin)
			{
				...
				List<CostoServicio> costosCmbBin = new ArrayList<CostoServicio>(); 
				if(tmpCmbBin.getCombo().getCostosCombo().size()>0)
				{
					for(CostoServicio cs:tmpCmbBin.getCombo().getCostosCombo()) 
					{
						costosCmbBin.add(cs);
					}
					comboBin.setCostosCombo(costosCmbBin);
	...
	{ //System.out.println("Entro a getCotizacionPendByDateN()");
		...
		cotizacionList = getEntityManager()
				.createQuery(
						...
						+ "	WHERE c.fechaIngreso BETWEEN :fch1 AND :fch2 and (c.estado=:estad1 OR c.estado=:estad2)  and c.cotizacionComboBin=null AND (c.ignorar is NULL OR c.ignorar!='si') order by c.id ASC")//.setParameter("suc", sucursalFlt)
			...
			cotizacionList = getEntityManager()
					.createQuery(
							"SELECT c FROM CotizacionComboApa c "
									+ "	WHERE "
									+ fltFch
									+ " AND (c.estado = 'PEN' OR c.estado='COT' OR c.estado='PRE' OR c.estado='TFN')  AND c.cotizacionComboBin = NULL AND (c.ignorar is NULL OR c.ignorar!='si') "+fltSuc+"  ORDER BY c.id ASC")
					.setParameter("suc", sucursalFlt)
					.setParameter("fch1", getFechaPFlt1())
					.setParameter("fch2", getFechaPFlt2()).getResultList();
			...
			cotizacionList = getEntityManager()
					.createQuery(
							"SELECT c FROM CotizacionComboApa c "
									...
									+ "AND (c.estado = 'PEN' OR c.estado='COT' OR c.estado='PRE' OR c.estado='TFN')  AND c.cotizacionComboBin = NULL AND (c.ignorar is NULL OR c.ignorar!='si') "+fltSuc+" ) ORDER BY c.id DESC")
					.setParameter("suc", sucursalFlt)
			...
			cotizacionList = getEntityManager()
					.createQuery(
							"SELECT c FROM CotizacionComboApa c "
									...
									+
									" CONCAT(UPPER(TRIM(c.cliente.nombres)),' ',UPPER(TRIM(c.cliente.apellidos))) LIKE :nom )"
									+ "AND (c.estado = 'PEN' OR c.estado='COT' OR c.estado='PRE' OR c.estado='TFN')  AND c.cotizacionComboBin = NULL AND (c.ignorar is NULL OR c.ignorar!='si')  ORDER BY c.id ASC")
					.setParameter("nom", "%"+this.nomCoinci.toUpperCase().trim()+"%")
					.getResultList();
	...
	public void fillComboList(ComboAparato combo) {
		...
		if (selCmbsList.size() >= 3) {
			...
			//comboAdd.setItemsCombo(combo.getItemsCombo()!=null?combo.getItemsCombo():null); comentado el 27/07/2017
			...
			/*//para evitar comportamiento lazy y no registre el producto de la categoria
			List<ItemComboApa> itemsCmbDb = getEntityManager().createQuery("FROM ItemComboApa i where i.combo.id=:idCombo").setParameter("idCombo", combo.getId()).getResultList();*/
			
			for(ItemComboApa item:combo.getItemsCombo())
			{
				ItemComboApa itemCm = new ItemComboApa();
				itemCm.setCategoria(item.getCategoria());
				itemCm.setProducto(item.getProducto());
				itemCm.setCantidad(item.getCantidad());
				itemCm.setPrincipal(item.isPrincipal());
				itemCm.setCombo(item.getCombo());
				itemCm.setDescripcion(item.getDescripcion());
				itemCm.setSelCategoria(item.getSelCategoria());
				itemCm.setGeneraRequisicion(item.isGeneraRequisicion());
							
				comboAdd.getItemsCotizados().add(itemCm);
			}
	...
	public void setComboVender(ComboAparato cmbApa) 
	{
		...
		System.out.println("Combo aparato seleecionado para vender "+ cmbApa.getNombre());
		System.err.println("Trata de setear en base al combo aparato");
		
		cmbApa.setResaltarColor(true);
			
		setComboVta(cmbApa);
		
		itemsComboApa = new ArrayList<ItemComboApa>();
			
		if (cmbApa.getPeriodoGarantia() != null && cmbApa.getPeriodoGarantia() > 0)
					setTieneGarantia(true);
	...
	public void preVenta()
	{
		...
		if(comboVta!=null)
		{
			...
			try
			{
				actualizarItemsCotizados();
			}
			catch (Exception e) 
			{
				e.printStackTrace();
				FacesMessages.instance().add(Severity.WARN,"Ocurrio un problema al actualizar los items");
				return;
			}
	...
	public boolean cotizarVenta() {
		...
		for (CotizacionCombos tmpCtCmbs : ctcmbs) 
		{
			...
			for (ItemComboApa itm : tmpCtCmbs.getCombo().getItemsCotizados()) 
			{
				...
				}*///dejado de utilizarse el 27/07/2017
				
				CotCmbsItems ctcmit = new CotCmbsItems();
				
				//ctcmit.setItem(itm);
				ctcmit.setProducto(itm.getProducto());//nuevo el 27/07/2017
				ctcmit.setCtCmbs(tmpCtCmbs);
				ctcmit.setTipoPrecio(itm.getTipoPrecio());
				ctcmit.setCantidad(itm.getCantidad());
				ctcmit.setPrincipal(itm.isPrincipal());
				ctcmit.setGeneraRequisicion(itm.isGeneraRequisicion());
				ctcmit.setNombreAdaptacion(itm.getNombreAdaptacion());
				ctcmit.setSelCategoria(itm.getSelCategoria());
				...
				if(itm.getSelCategoria()!=null && itm.getSelCategoria().equals("si"))// para evitar el problema q no se registre el producto en el item del combo
					itm.setProducto(null);
		...
		if (isBinaural()) 
		{
			...
			for (CotizacionCombos tmpCtCmbsBin : ctcmbsBin) 
			{
				...
				System.out.println("Persistí el comboBin con ID: "+ tmpCtCmbsBin.getCombo().getNombre());
				...
				for (ItemComboApa itm : tmpCtCmbsBin.getCombo().getItemsCotizados()) 
				{
					...
					CotCmbsItems ctcmit = new CotCmbsItems();
					//ctcmit.setItem(itm);
					ctcmit.setProducto(itm.getProducto());
					...
					if(itm.getSelCategoria()!=null && itm.getSelCategoria().equals("si"))
						itm.setProducto(null);
	...
	private CotizacionComboApa genCotizacionApa(boolean esBinaural,CotizacionComboApa cotizPadre) 
	{
		...
		/*for (CotizacionComboItem tmpItm : itemsCotizacion) { comentado el 27/07/2017
	...
	public void actualizarItemsCotizados() throws Exception
	{
		
		if(comboVta!=null)
		{
			for(ItemComboApa item:comboVta.getItemsCotizados())
			{
				for(CotCmbsItems itemCot:itemsCotizadosPorCombo.get(comboVta.getId()))
				{
					if(item.getIdItemCotizado()==itemCot.getId())
					{
						itemCot.setCantidad(item.getCantidad());
						itemCot.setProducto(item.getProducto());
						itemCot.setPrecioCotizado(item.getPrecioCotizado());
						itemCot.setTipoPrecio(item.getTipoPrecio());
	...
	public void actualizarItemsCotizadosBin() throws Exception
	{
		if(comboVtaBin!=null)
		{
			for(ItemComboApa item:comboVtaBin.getItemsCotizados())
			{
				for(CotCmbsItems itemCot:itemsCotizadosPorComboBin.get(comboVtaBin.getId()))
				{
					if(item.getIdItemCotizado()==itemCot.getId())
					{
						itemCot.setCantidad(item.getCantidad());
...
*/			
	...
	public Integer getIdCotizacionParaDuplicar() {
		return idCotizacionParaDuplicar;
	}

	public void setIdCotizacionParaDuplicar(Integer idCotizacionParaDuplicar) {
		this.idCotizacionParaDuplicar = idCotizacionParaDuplicar;
	}
